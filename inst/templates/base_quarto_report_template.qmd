...
::: {.content-visible when-format="pdf"}
```{=tex}
\subsubsection*{Suggested Citation}
```
:::

::: {.content-visible when-format="docx"}
### Suggested Citation {.unnumbered .unlisted}
:::

Stoughton, Elizabeth & Monchka, Barret. {*Data Linkage Quality Report: Social Allowances Management Information Network*}. Winnipeg, Manitoba: George & Fay Yee Centre for Healthcare Innovation; `r format(Sys.Date(), "%B %Y")`.
...
Researchers using this linked data should familiarize themselves with the record linkage methods employed, carefully assess the representativeness of the linked sample, and consider how linkage errors may impact the interpretation of study findings. The detailed information in the [Background](#sec-background) and [Methods](#sec-methods) sections will aid in interpreting the results of the linkage process. When disseminating research based on linked data, we encourage adherence to reporting guidelines to support critical evaluations of findings and promote research excellence.[@bohensky2011; @gilbert2017; @pratt2020]   

The [Results](#sec-results) section includes figures and tables with descriptive statistics that summarize the performance of the linkage algorithm and the quality of the linked data. To asses the suitability of the data for research purposes, consider the following recommendations:  

`r list_num = 1`

`r list_num`. **Examine the representativeness of the linked sample:** Use the column percentages in @tbl-linkage_rates to compare record characteristics with their distribution in the source data. Significant differences in these distributions may affect the generalizability of study findings. 
...
`r list_num`. **Explore variations in linkage rates over data acquisition dates:** Refer to @fig-linkage_rate_dist to identify shifts in data quality or variations in data collection practices over time. Differential linkage error with respect to acquisition dates leads to a non-random sample.
