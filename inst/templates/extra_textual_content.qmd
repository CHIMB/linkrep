...
Linkage was performed using the reclin2 package (version 0.5.0) in R.[@reclin2pkg] Records were linked through an iterative process using a combination of deterministic and probabilistic steps`r if (!is.null(algorithm_summary_table)) " (@tbl-algorithm_summary)"`. For probabilistic steps, the Fellegi-Sunter model @fellegisunter with extensions were used to generate match weights for each candidate record pair that met blocking constraints.\
