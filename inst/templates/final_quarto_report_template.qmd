...
threshold_plot_titles <- params$threshold_plot_titles

flow_chart_path <- params$flow_chart_path
...
Elizabeth Stoughton, Cole Chuchmach, Barret Monchka. *`r params$report_title`: `r params$report_subtitle`*. Winnipeg, Manitoba: George & Fay Yee Centre for Healthcare Innovation; `r format(Sys.Date(), "%B %Y")`.
...
This report details the methods used to link records from `r params$left_dataset_name` to those in `r params$right_dataset_name``r ifelse(is.null(params$num_records_right_dataset), "", sprintf(" (N = %s)", params$num_records_right_dataset))`, presents the findings from this linkage, and offers guidance on evaluating the quality of the linkage process.

We used an iterative approach consisting of both deterministic and probabilistic matching techniques, that linked `r params$num_records_linked` out of `r params$num_records_left_dataset` records from `r params$left_dataset_name` for an overall linkage rate of `r params$overall_linkage_rate`% (see **@tbl-linkage_rates**).

```{r, results='asis'}
# Get the performance measures data frame
performance_measures_df <- params$performance_measures_df

if (!is.null(performance_measures_df) && all(c("sensitivity", "positive_predictive_value") %in% names(performance_measures_df)) && !is.null(params$num_pairs_non_missing_ground_truth) && !is.null(params$percent_non_missing_ground_truth)) {
  #print
  sens_val         <- round(performance_measures_df$sensitivity, 1)
  ppv_val          <- round(performance_measures_df$positive_predictive_value, 1)
  gt_agreement_n   <- params$num_pairs_non_missing_ground_truth
  gt_agreement_pct <- params$percent_non_missing_ground_truth
  ground_truth_str <- params$ground_truth

  # Create the string to print
  performance_string <- paste0('The probabilistic linkage steps attained ', sens_val, '% sensitivity and ', ppv_val, '% positive predictive value (see **@tbl-performance_measures_tbl**), estimated through agreement on ', ground_truth_str, ' among individuals with non-missing values for those fields (N=', gt_agreement_n, ', ', gt_agreement_pct, '%).')
  
  # Print it
  cat(performance_string)
}
```

To discuss the findings of this report, the potential impact of linkage errors on a research study, or to request guidance on adjusting for linkage errors in a data analysis, you may schedule a consultation with CHI by visiting <https://chimb.ca> and following the instructions to request a consultation. For additional questions regarding this data linkage, please e-mail [data.linkage\@umanitoba.ca](mailto:data.linkage@umanitoba.ca).
...
The [**Results**](#sec-results) section includes figures and tables that summarize the performance of the linkage algorithm and the quality of the linked data. To assess the suitability of the data for research purposes, consider the following recommendations:

\setlength{\parskip}{0.5em}

`r list_num = 1`

`r list_num`. **Examine the representativeness of the linked sample:** Use the column percentages in **@tbl-linked_data_repr** to compare the distribution of characteristics between the source and linked data. Significant differences in these distributions may affect the generalizability of study findings. `r list_num = list_num + 1`

`r list_num`. **Assess potential biases in the linkage process:** Utilize the row percentages in **@tbl-linkage_rates** to examine the proportion of unlinked records, stratified by sociodemographic and other characteristics. For example, a significantly lower linkage rate among females compared to males may introduce sampling error into a research study. `r list_num = list_num + 1`

`r if(!is.null(linkage_rates_over_time_plot)) "::: {.content-visible}" else "::: {.content-hidden}"`
`r list_num`. **Explore variations in linkage rates over data acquisition dates:** Refer to `r if(!is.null(linkage_rates_over_time_plot)) "@fig-linkage_rate_dist"` to identify shifts in data quality or variations in data collection practices over time. Temporal inconsistencies in data linkage performance will result in a non-random sample.
...
`r list_num`. **Critically review the multi-step linkage algorithm employed:** Analyze the parameters used in each step of the algorithm `r if(!is.null(algorithm_summary_table)) "**(@tbl-algorithm_summary)**"`, including the matching technique (deterministic or probabilistic), acceptance threshold, and variables considered, to assess how confidently linked records can be attributed to the same individual. For example, exact matches on multiple fields generally reduce the risk of linkage error, while heavy reliance on approximate string matching can introduce noise into the linked data. Similarly, higher acceptance thresholds and the use of variables with strong discriminative power (e.g., last name or unique personal identifiers) tend to improve linkage accuracy, compared to lower thresholds and variables with weaker discriminative ability (e.g., sex/gender or postal code).
`r if(!is.null(algorithm_summary_table)) list_num = list_num + 1`
:::
...
```{r data flow diagram and linked data orientation before, results='asis'}
if(is.null(flow_chart_path)){
  if(requires_landscape_page(linked_data_repr_table)){
...
`r if(!is.null(flow_chart_path)) "::: {.content-visible}" else "::: {.content-hidden}"`
...
```{r data flow diagram, ft.align="left", results='asis'}
plot <- flow_chart_path
caption <- paste0('Data flow diagram showing exclusions made to the ', params$left_dataset_name, ' prior to performing linkage.')
cat(paste0("![", caption, "](", plot, "){width=80%}"), "\n\n")
```

```{r linked data page orientation before, results='asis'}
if (!is.null(linked_data_repr_table)){
  if (requires_landscape_page(linked_data_repr_table)){
    set_page_width("landscape")
  }
  else{
...
#| tbl-cap: !expr paste0("Characteristics of records in ", params$left_dataset_name, " (source) and those that linked to ",  params$right_dataset_name) 
...
#| tbl-cap: !expr paste0("Stratified linkage rates for records in ", params$left_dataset_name, " (source) that linked to ",  params$right_dataset_name) 
...
for(i in 1:length(threshold_plots)){
  ...
  title <- threshold_plot_titles[i]
  
  # Add a header above each plot, then print the plot
  cat(paste0("### ", title, "\n"))
...
## Linkage Algorithm Performance Summary
...
#| tbl-cap: !expr paste0("Missingness of select variables within the ", params$left_dataset_name)
