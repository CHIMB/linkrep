...
```{r}
if (all(c("SENS", "PPV") %in% names(performance_measures_table)) && !is.null(num_pairs_non_missing_ground_truth) && !is.null(percent_non_missing_ground_truth)) {
  sens_val         <- performance_measures_table$SENS
  ppv_val          <- performance_measures_table$PPV
  gt_agreement_n   <- num_pairs_non_missing_ground_truth
  gt_agreement_pct <- percent_non_missing_ground_truth
