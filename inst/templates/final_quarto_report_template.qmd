...
if (!is.null(performance_measures_df) && all(c("sensitivity", "positive_predictive_value") %in% names(performance_measures_df)) && !is.null(params$num_pairs_non_missing_ground_truth) && !is.null(params$percent_non_missing_ground_truth)) {
  ...
  performance_string <- paste0('The probabilistic linkage steps attained ', sens_val, '% sensitivity and ', ppv_val, '% positive predictive value (see **@tbl-performance_measures_tbl**), estimated through agreement on ', ground_truth_str, ' among individuals with non-missing values (N=', gt_agreement_n, ', ', gt_agreement_pct, '%).')
...
```{r study flow diagram, ft.align="left", results='asis'}
plot <- flow_chart_path
caption <- paste0('Study flow diagram of data from ', params$left_dataset_name, ' and ', params$right_dataset_name)
cat(paste0("![", caption, "](", plot, "){width=70%}"), "\n\n")
```
