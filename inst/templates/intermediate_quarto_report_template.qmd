...
params: 
  ...
  algorithm_summary_table_names: null
  ...
  overall_linkage_rate_min: null
  overall_linkage_rate_max: null
...
number_of_algorithms <- length(params$algorithm_summary_table_path)

algorithm_summary_table <- list()
for(i in 1:number_of_algorithms){
  algorithm_summary_table[[i]] <- read_data(params$algorithm_summary_table_path[[i]], "algorithm_summary_tabl_path")
}

algorithm_summary_table_names <- params$algorithm_summary_table_names
...
```{r, message = NA}
#message(params$algorithm_summary_table_path)
```
...
This report details the methods used to link records from `r params$left_dataset_name` to those in `r params$right_dataset_name`, presents the findings from this linkage, and offers guidance on evaluating the quality of the linkage process.

We evaluated `r number_of_algorithms` alternative linkage algorithms ([**Summaries**](#alg-summaries), [**Performance**](#perf-summaries)), each consisting of both deterministic and probabilistic matching techniques. Across considered algorithms, linkage rates ranged from `r params$overall_linkage_rate_min`% to `r params$overall_linkage_rate_max`% (see **@tbl-linkage_rates**).
...
`r list_num`. **Examine the representativeness of the linked sample:** Use the column percentages in **@tbl-linked_data_repr** to compare the distribution of characteristics between the source and linked data. Significant differences in these distributions may affect the generalizability of study findings. 
`r list_num = list_num + 1` 


`r list_num`. **Assess potential biases in the linkage process:** Utilize the row percentages in **@tbl-linkage_rates** to examine the proportion of unlinked records, stratified by sociodemographic and other characteristics. For example, a significantly lower linkage rate among females compared to males may introduce sampling error into a research study.
`r list_num = list_num + 1`   
...
for(table in algorithm_summary_table){
  if (!is.null(table)){
...
## Summaries of Considered Linkage Algorithms {#alg-summaries}

```{r algorithm summary table, ft.align="left", results='asis'}

if (!is.null(algorithm_summary_table)){
  for(i in 1:length(algorithm_summary_table)){
    # Get the table
    table <- algorithm_summary_table[[i]]
    
    # Assuming each algorithm has a name stored as names(algorithm_summary_table)
    algorithm_name <- algorithm_summary_table_names[i]
    
    # Table number dynamic generation
    table_num <- i + 2
    
    # Create a dynamic caption
    dynamic_caption <- paste0(
      "Table ", table_num, ": Summary of the multi-step algorithm \"", algorithm_name, "\" for",
...
`r if(is.null(threshold_plots) & !is.null(performance_measures_table)) "::: {.content-visible}" else "::: {.content-hidden}"`

## Classification Performance of Considered Linkage Algorithms {#perf-summaries}
